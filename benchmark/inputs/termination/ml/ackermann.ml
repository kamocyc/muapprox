...
let rec ack prev depth m n k =
  print_endline @@ show (m, n);
  print_endline @@ string_of_int depth;
  if not (lt (m, n) prev) then failwith "a";
  if m = 0 then k (n + 1)
  else if n = 0 then ack (m, n) (depth + 1) (m-1) 1 k
  else ack (m, n) (depth + 1) m (n-1) (fun r ->
    (* print_endline (show_3 (m, n, r)); *)
    ack (m, n) (depth + 1) (m-1) r k
  )
...
let () =
  ...
  let n = read_int () in
  if n>0 && m>0 then
    ack (m+1, n) 0 m n (fun r -> print_int r)
  else
    print_int 0
