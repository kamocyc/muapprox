...
let hes_mu_forall = HesLogic.mk_hes
    [
      Predicate.Mu, (Ident.Pvar "F"), [Ident.Tvar "X", T_int.SInt],
      (Formula.mk_or
         (Formula.mk_atom (T_int.mk_lt varX (T_int.mk_int (Bigint.of_int 0) Dummy) Dummy) Dummy)
         (Formula.mk_or
            (Formula.mk_atom
               (Atom.mk_app varF [T_int.mk_add varX (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy] Dummy)
               Dummy)
            (Formula.mk_atom (Atom.mk_app varF [T_int.mk_sub varX (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy] Dummy) Dummy)
            Dummy)
         Dummy)
    ]
    entry_forall
...
let hes_mu_for_approx = HesLogic.mk_hes
    [
      (
        Predicate.Mu, (Ident.Pvar "F"), [Ident.Tvar "X", T_int.SInt],
        (Formula.mk_imply
           (Formula.mk_atom (T_int.mk_geq varX (T_int.mk_int (Bigint.of_int 100) Dummy) Dummy) Dummy)
      ...
      (
        ...
        (Formula.mk_and
           ...
           (Formula.mk_and
              (Formula.mk_atom
                 (Atom.mk_app varG [T_int.mk_add varX (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy] Dummy)
                 Dummy)
              (Formula.mk_atom (Atom.mk_app varG [T_int.mk_sub varX (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy] Dummy) Dummy)
              Dummy)
           Dummy)
...
let _ = run_test_tt_main begin
    "solve_onlyforall" >::: (gen_tests ~pp_diff:pp_diff solve_onlyforall_tests)
  end

let _ = run_test_tt_main begin
    "simplesolve_onlynu_onlyforall" >::: (gen_tests ~pp_diff:pp_diff simplesolve_onlynu_onlyforall_tests)
  end

let _ = run_test_tt_main begin
    "simplesolve_onlymu_onlyexists" >::: (gen_tests ~pp_diff:pp_diff simplesolve_onlymu_onlyexists_tests)
  end
