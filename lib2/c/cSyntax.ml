...
module Define = struct
  ...
  let rec subst_of_defines = function
    ...
    | DEF (key, value) :: tail ->
      ...
      let term = T_int.mk_int (Bigint.of_int value) Dummy in
...
module rec Ctl : sig
  ...
  let rec string_of = function
    ...
    | AP fml -> Printf.sprintf "AP(%s)" (str_of_formula fml)
  ...
  let rec simplify = function
    ...
    | AP fml -> AP (Formula.simplify fml)
  ...
  and simplify_neg = function
    ...
    | AP fml -> AP (Formula.simplify_neg fml)
...
module rec Ltl : sig
  ...
  let is_unop = function F _ | G _ -> true | _ -> false
  ...
  let rec string_of ?(priority=0) = function
    | F phi ->
      ...
      |> add_bracket priority Priority.atom
    | G phi ->
    ...
    | U (phi1, phi2) ->
      ...
      |> add_bracket priority Priority.binary_u
    | OR (phi1, phi2) ->
    ...
    | P fml ->
      let priority' =
        if priority = Priority.atom then
          Convert.PrinterHum.Priority.unary_neg
        else if priority = Priority.binary_and then
          Convert.PrinterHum.Priority.binary_and
        else if priority = Priority.binary_or then
          Convert.PrinterHum.Priority.binary_or
        else
          priority
      in
      Printf.sprintf "%s" (str_of_formula ~priority:priority' fml)
      |> add_bracket priority Priority.atom

  let rec get_fv = function
    | G phi | F phi | NEG phi -> get_fv phi
  ...
  and simplify_neg = function
    ...
    | P fml -> P (Formula.simplify_neg fml)
...
module rec BuchiAutomaton : sig
  ...
  type 'a t = BA of int * int * int Core.Set.Poly.t * (int * 'a) list array

  val mk_ba: states:int -> initial_state:int -> final_states:int Core.Set.Poly.t -> transition:(int * 'a) list array -> 'a t
  val init_ba: states:int -> initial_state:int -> final_states: int list -> transition: (int * int * 'a) list -> 'a t

  val let_ba: 'a t -> int * int * int Core.Set.Poly.t * (int * 'a) list array

  val get_state_count: 'a t -> int
  val get_initial_state_id: 'a t -> int
  val get_final_state_ids: 'a t -> int Core.Set.Poly.t
  ...
  let init_ba ~states ~initial_state ~final_states ~transition =
    let final_states = Core.Set.Poly.of_list final_states in
    let graph = (Array.init states (fun _ -> [])) in
    List.iter
      (fun (from_state_id, to_state_id, label_or_cond) ->
  ...
  let string_of ~printer = function
    | BA (states, initial_state, final_states, transition) ->
      Printf.sprintf "states: %d\ninitial state: %s\nfinal states: %s\ntransition:\n%s"
        ...
        (Core.Set.Poly.to_list final_states
         ...
         |> (List.mapi
               (fun from_id toes ->
                  Printf.sprintf " %s%s: %s"
                    ...
                    (
                      if Core.Set.Poly.mem final_states from_id then
...
module rec HMES : sig
  ...
  let mk_hmes funcs entry_pvar = Hmes (funcs, entry_pvar)
  let mk_func fix pvar fml = fix, pvar, fml
  let let_hmes = function Hmes (funcs, entry_pvar) -> funcs, entry_pvar

  let string_of = function Hmes (funcs, entry_pvar) ->
    Printf.sprintf "%s\ns.t.\n%s"
      (Ident.name_of_pvar entry_pvar)
      (List.map
         (fun (fix, pvar, fml) ->
            Printf.sprintf "%s =%s %s"
              (Ident.name_of_pvar pvar)
              (Convert.PrinterHum.str_of_fixpoint fix)
              (HMESFormula.string_of fml)
         ) funcs
       |> String.concat ";\n")

  let get_ftv = function Hmes (funcs, _) ->
    List.fold_left
      (fun vars (_, _, fml) -> Variables.union vars (HMESFormula.get_ftv fml))
      Variables.empty
      funcs
end
and HMESFormula : sig
...
and HMESAtom : sig
  ...
  let atom_of = function
    | True _ -> Atom.mk_true Dummy
    | False _ -> Atom.mk_false Dummy
    | Pvar pvar -> Atom.mk_app (Predicate.mk_var pvar []) [] Dummy
    | PredApp (sym, args) -> Atom.mk_app (Predicate.mk_psym sym) args Dummy
  ...
  let string_of ?(priority=0) = function
    ...
    | Pvar pvar ->
      ...
      |> add_bracket priority HMESPriority.atom
    | PredApp (sym, args) ->
      Convert.PrinterHum.str_of_atom (Atom.mk_app (Predicate.mk_psym sym) args Dummy)
      |> add_bracket priority HMESPriority.atom
...
module rec Statement : sig
  ...
  let rec string_of ?(indent=0) = function
    ...
    | VARDECL (varname, sort) ->
      Printf.sprintf "%s%s %s;"
        (make_indent indent)
        (Convert.PrinterHum.string_of_sort sort)
        varname
    ...
    | RETURN_INT term -> Printf.sprintf "%sreturn %s;" (make_indent indent) (Convert.PrinterHum.str_of_term term)
  ...
  let rec get_label_references_rep ?(nxt_stmt=Statement.mk_nop ()) ?(res=Hashtbl.create 10) = function
    | IF (_, stmt1, stmt2)
    | NONDET (stmt1, stmt2) ->
    ...
    | LABEL label_name ->
      Hashtbl.add res label_name nxt_stmt;
      res
    | VARDECL _
    | GOTO _
    | RETURN_INT _
  ...
  let rec get_read_vars_rep label_refs used_labels = function
    ...
    | VARDECL _ -> assert false (* TODO *)
    | GOTO label_name ->
      if Variables.is_mem label_name used_labels then
        Variables.empty, used_labels
      else
        Hashtbl.find label_refs label_name
...
module rec Init : sig
  ...
  let update_state state = function
    ...
    | ASSUME _ -> state
    | NONDET_ASSIGN varname ->
      let term = Term.mk_var (Ident.Tvar varname) T_int.SInt Dummy in
      State.update varname term state
  ...
  let is_already_assigned inits varname =
    List.exists
      (function
          ASSIGN (varname', _) | NONDET_ASSIGN varname' -> varname' = varname
        | ASSUME _ -> false)
      inits
...
and State : sig
  ...
  val bounds_of: t -> (Ident.tvar * Sort.t) list
  ...
  let of_variables variables =
    let state =
      ...
      |> List.map (fun varname -> varname, ref (Term.mk_var (Ident.Tvar varname) T_int.SInt Dummy))
    in
    STATE state
  ...
  let bounds_of = function STATE state ->
    List.map (fun (varname, _) -> Ident.Tvar varname, T_int.SInt) state
...
module FunDecl : sig
  ...
  let rec get_next_funnames_rep stmt res =
    if Statement.is_call_assign stmt then
      ...
      |> List.fold_left
        (fun res stmt' -> get_next_funnames_rep stmt' res)
        res
  ...
  let rec is_non_recursive_rep fundecls fundecl ancestors used =
    ...
    |> List.fold_left
      (fun (res, used) funname' ->
         let fundecl' = find_fundecl funname' fundecls in
         if not res then
           false, used
         else if Variables.is_mem funname' ancestors then
           true, used
         else if Variables.is_mem funname' used then
           true, used
         else
           is_non_recursive_rep fundecls fundecl' (Variables.add funname' ancestors) (Variables.add funname' used)
      )
      (true, used)

  let is_non_recursive fundecls fundecl =
    let funname = get_funname fundecl in
    let res, _ = is_non_recursive_rep fundecls fundecl (Variables.of_varname funname) (Variables.of_varname funname) in
    res

  let string_of_args args =
    List.map
      (fun (varname, sort) ->
         let sort_str = Convert.PrinterHum.string_of_sort sort in
         Printf.sprintf "%s %s" sort_str varname
      ) args
    |> String.concat ", "
