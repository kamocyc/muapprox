...
%start toplevel
%type <(string * string * Ast.Logic.Formula.t) list> toplevel
%%
...
state:
    id=ID CORON SKIP { [id, id, Formula.mk_atom (Atom.mk_true Dummy) Dummy] }
...
formula_or:
    fml=formula_and { fml }
  | fml1=formula_and OR fml2=formula_or { Formula.mk_or fml1 fml2 Dummy }

formula_and:
    fml=formula_unary { fml }
  | fml1=formula_unary AND fml2=formula_and { Formula.mk_and fml1 fml2 Dummy }

formula_unary:
    fml=formula_atom { fml }
  | NOT fml=formula_unary { Formula.mk_neg fml Dummy }

formula_atom:
    id=ID { Formula.mk_atom (Atom.mk_app (Predicate.Var (Ident.Pvar id, [])) [] Dummy) Dummy }
