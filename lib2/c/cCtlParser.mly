...
StatementGeneral:
  ...
  | DOCHECK LPAREN RPAREN SEMI { Statement.mk_assign "check" (T_int.mk_int (Bigint.of_int 1) Dummy) }
...
FormulaAnd:
    left=FormulaOr AND right=FormulaAnd { Formula.mk_and left right Dummy }
  | fml=FormulaOr { fml }

FormulaOr:
    left=FormulaNeg OR right=FormulaOr { Formula.mk_or left right Dummy }
  | fml=FormulaNeg { fml }
...
T_intMultDivMod:
    t=T_intNeg { t }
  | t1=T_intNeg ASTERISK t2=T_intMultDivMod { T_int.mk_mult t1 t2 Dummy }
