...
module StatementEnv = struct
  ...
  let init init items = ENV (init, List.map (fun (key, value) -> key, ref value) items)

  let exists stmt = function
      ENV (_, env) ->
      List.exists (fun (stmt', _) -> stmt' == stmt) env
  ...
  let remove stmt = function
      ENV (init, env) ->
      ENV (init, env |> List.filter (fun (stmt', _) -> stmt' != stmt))

  let get stmt = function
      ENV (init, env) ->
      match List.find_opt (fun (stmt', _) -> stmt' == stmt) env with
      | Some (_, item) -> !item
      | None -> init
  ...
  let keys = function
      ENV (_, env) ->
      List.map (fun (key, _) -> key) env

  let values = function
      ENV (_, env) ->
      List.map (fun (_, value) -> !value) env
    ...
    | [nxt_stmt] ->
      List.iter
        (fun stmt' ->
           ...
           |> List.iter (fun stmt_ref -> if !stmt_ref == stmt then stmt_ref := nxt_stmt))
        stmts;
      if entry == stmt then
        nxt_stmt
      else
  ...
  let replace_stmt ~entry new_stmts =
    ...
    |> List.fold_left
      (fun used stmt ->
         ...
         |> List.fold_left
           (fun used nxt_stmt ->
              if List.exists (fun r -> r == nxt_stmt) used then
                used
...
module ReadGraph = struct
  ...
  (* let rg_rev = function RG rg ->
     ...
     |> List.fold_left
      ...
      (StatementEnv.mk_empty [])

     let rgenv_rev = function RGENV rgenv ->
     let rgdata =
      Env.entries rgenv
      |> List.map
        (fun (tvar, rg) -> tvar, rg_rev rg)
     in
  ...
  let rec rc_of_term_rep varname term res =
    if Term.is_var term then
      ...
      if Ident.name_of_tvar tvar = varname then
        res + 1
      else
        res
    else if Term.is_funapp term then
      let _, args, _ = Term.let_funapp term in
      List.fold_left
        (fun res term -> rc_of_term_rep varname term res)
        res
        args
    else
  ...
  let rc_of_atom_rep varname atom res =
    if Atom.is_app atom then
      ...
      failwith @@ Printf.sprintf "rc_of_atom_rep: not implemented for: %s" (Convert.PrinterHum.str_of_atom atom)
  ...
  let rec rc_of_formula_rep varname fml res =
    if Formula.is_atom fml then
      ...
      if List.exists (fun (tvar, _) -> Ident.name_of_tvar tvar = varname) bounds then
        0
  ...
  let rec get_rg_dfs varname prev_env stmt res: rg_t =
    let rs =
      get_next_statements stmt
      |> List.fold_left
        (fun rs nxt_stmt -> rg_get nxt_stmt res |> merge rs)
        empty
    in
  ...
  and get_rg_dfs_update varname prev_env stmt res rs =
    ...
    if is_updated rs0 rs then
      ...
      |> List.fold_left
        (fun res prev_stmt -> get_rg_dfs varname prev_env prev_stmt res)
        res
    else
      res
  ...
  let get_rg varname prev_env stmts: rg_t =
    List.fold_left
      (fun rg stmt -> get_rg_dfs varname prev_env stmt rg)
      rg_empty
      stmts
  ...
  let get_rgenv spec_fv prev_env stmt: rgenv_t =
    ...
    let rgdata =
      ...
      |> List.fold_left
        (fun rgdata varname ->
           if Variables.is_mem varname spec_fv then
             rgdata
           else
...
module RemoveAssign : sig
  ...
  let remove_stmt stmt_key stmt_keys (entry_stmt_key, new_stmts) =
    ...
    | [next_stmt_key] ->
      if stmt_key != next_stmt_key then
  ...
  let sub_assign_one init_state rgenv stmt_key stmt_keys (entry_stmt_key, new_stmts) =
    if StatementEnv.exists stmt_key new_stmts then
      ...
      |> Variables.to_list
      |> List.fold_left
        (fun (entry_stmt_key, new_stmts) varname ->
           let tvar = Ident.Tvar varname in
           if ReadGraph.rgenv_mem tvar rgenv then
             (* TODO: simplify terms *)
             ...
             let init_value =
               ...
               if ReadGraph.mem stmt_key rc then
                 if State.mem varname init_state then
                   ...
                   assert(Term.get_ftv term |> Core.Set.Poly.is_empty);
                   T_LIT term
  ...
  let remove_unused_assign_one rgenv stmt_key stmt_keys (entry_stmt_key, new_stmts) =
    if StatementEnv.exists stmt_key new_stmts then
      ...
      if is_assign stmt || is_nondet_assign stmt then
        ...
        && ReadGraph.rgenv_get tvar rgenv |> ReadGraph.rg_get !nxt_stmt_key = Never then
          remove_stmt stmt_key stmt_keys (entry_stmt_key, new_stmts)
        else
          (* case: <tvar> \in <phi> *)
          entry_stmt_key, new_stmts
  ...
  let sub_assign is_print_for_debug stmts inits spec_fv entry_stmt =
    let new_stmts = Util.mk_replacer stmts in
    let prev_env = Util.get_prev_env entry_stmt in
    let rgenv = ReadGraph.get_rgenv spec_fv prev_env entry_stmt in
    if is_print_for_debug then
      Debug.print @@ Printf.sprintf "rgenv:\n%s\n\n" (ReadGraph.string_of_rgenv entry_stmt rgenv);
    let init_state = State.of_inits inits in
    stmts
    |> List.fold_left
      (fun (entry_stmt, new_stmts) stmt ->
         sub_assign_one init_state rgenv stmt stmts (entry_stmt, new_stmts))
      (entry_stmt, new_stmts)
    |> (fun (entry_stmt, new_stmts) ->
        Util.replace_stmt ~entry:entry_stmt new_stmts)

  let remove_unused_assign stmts spec_fv entry_stmt =
    let new_stmts = Util.mk_replacer stmts in
    let prev_env = Util.get_prev_env entry_stmt in
    let rgenv = ReadGraph.get_rgenv spec_fv prev_env entry_stmt in
    let entry_stmt, _ =
      List.fold_left
        (fun (entry_stmt, new_stmts) stmt -> remove_unused_assign_one rgenv stmt stmts (entry_stmt, new_stmts))
        (entry_stmt, new_stmts)
        stmts
    in
    Util.replace_stmt ~entry:entry_stmt new_stmts
...
module RemoveTrailingNoValueChanged : sig
  ...
  let rec dfs prev_env stmt used =
    if StatementEnv.exists stmt used then
      ...
      StatementEnv.get stmt prev_env
      |> List.fold_left
        (fun used prev_stmt -> dfs prev_env prev_stmt used)
        used

  let optimize_stmt spec_fv entry_stmt =
    let stmts = get_all_statements entry_stmt in
    let prev_env = Util.get_prev_env entry_stmt in
    let noreplace_stmts =
      stmts
      |> List.fold_left
        (fun used stmt ->
           if is_assume stmt
           || not (Variables.is_empty (Variables.inter (get_written_vars stmt) spec_fv)) then
