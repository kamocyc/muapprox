%{
  ...
  let formula_of_term term = Formula.mk_atom (T_bool.mk_neq term (T_int.mk_int (Bigint.of_int 0) Dummy) Dummy) Dummy
  let term_of_string str = let varname = Printf.sprintf "\"%s\"" str in Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy
  let is_nondet term =
...
Statement:
  ...
  | IF LPAREN cond_assign_stmt=FormulaAssignOne RPAREN t_stmt=StatementIfT {
      ...
      let term = Term.mk_var tvar T_int.SInt Dummy in
      let cond_fml = Formula.mk_atom (T_bool.mk_neq term (T_int.mk_int (Bigint.of_int 0) Dummy) Dummy) Dummy in
      stmt_of_statements [
        cond_assign_stmt;
          ...
          Statement.mk_if cond_fml
            (stmt_of_statements [
              ...
              Statement.mk_assign step_var
                (T_int.mk_add
                  (Term.mk_var tvar T_int.SInt Dummy)
...
IntVarDeclOne:
    varname=ID
  | ASTERISK varname=ID { varname, None }
...
FormulaNeg:
    NOT fml=FormulaNeg { Formula.mk_neg fml Dummy }
  | fml=FormulaAtom { fml }
...
FormulaAtom:
  ...
  /* | n=INTL { T_int.mk_int (Bigint.of_int n) Dummy |> formula_of_term }
  | varname=ID { Term.mk_var (Ident.Tvar varname) T_int.SInt Dummy |> formula_of_term } */
...
T_intMultDivMod:
    t=T_intUnary { t }
  | t1=T_intUnary ASTERISK t2=T_intMultDivMod { T_int.mk_mult t1 t2 Dummy }
  | t1=T_intUnary DIV t2=T_intMultDivMod { T_int.mk_div t1 t2 Dummy }
  | t1=T_intUnary MOD t2=T_intMultDivMod { T_int.mk_mod t1 t2 Dummy }

T_intUnary:
    t=T_intParen { t }
  | Cast t=T_intUnary { t }
  | MINUS t=T_intUnary { T_int.mk_unary_neg t Dummy }
...
T_intAtom:
  ...
  | funname=ID LPAREN args=Arguements RPAREN {
    Term.mk_funapp (FunCall funname) args Dummy
  }
  | ADDR varname=ID { Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy }
  | ASTERISK varname=ID { Term.mk_var (Ident.Tvar varname) T_int.SUnrefInt Dummy }
  /* sizeof(int) -> 4 */
  | SIZEOF LPAREN INT RPAREN { T_int.mk_int (Bigint.of_int 4) Dummy }
  /* --x, ++x */
  | MINUSMINUS varname=ID { Term.mk_funapp (FunCall "#dec") [Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy] Dummy }
  | PLUSPLUS varname=ID { Term.mk_funapp (FunCall "#inc") [Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy] Dummy }
  /* x--, x++ */
  | varname=ID MINUSMINUS { T_int.mk_add (Term.mk_funapp (FunCall "#dec") [Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy] Dummy) (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy }
  | varname=ID PLUSPLUS { T_int.mk_add (Term.mk_funapp (FunCall "#inc") [Term.mk_var (Ident.Tvar varname) T_int.SRefInt Dummy] Dummy) (T_int.mk_int (Bigint.of_int 1) Dummy) Dummy }
  /* "hoge" */
  | str=STRINGL { term_of_string str }
  /* "hoge" "fuga" */
  | str1=STRINGL str2=STRINGL { term_of_string (str1 ^ str2) }
  /* nondet() */
  | NONDET LPAREN RPAREN { Term.mk_funapp (FunCall funname_nondet) [] Dummy }
