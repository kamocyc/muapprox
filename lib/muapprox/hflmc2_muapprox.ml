...
let check_validity_full coe1 coe2 _ timeout (is_print_for_debug : bool) hes = 
  ...
  let rec go coe1 coe2 =
    ...
    let result, result' = solve_onlynu_onlyforall () timeout is_print_for_debug nu_only_hes in
    match result with
    | Status.Valid -> (Status.Valid, result')
    | _ -> begin
      let nu_only_dual_hes = elim_mu_exists coe1 coe2 dual_hes in
      let dual_result, dual_result' = solve_onlynu_onlyforall () timeout is_print_for_debug nu_only_dual_hes in
