...
let get_type (phi : tv_expression): R.ptype =
  let rec go (phi : tv_expression): R.ptype =
    match phi with
    | PUnit -> R.TUnit
    | PVar x -> x.ty
    | PIf (a, p1, p2) ->
    ...
    | Bool _ -> TBool
    | PLambda (args, body) ->
      let tys = List.map (fun a -> a.Id.ty) args in
      let ty = go body in
      let rec t ty = function
        | [] -> ty
        | x::xs -> R.TFunc (x, t ty xs)
      in
      t ty tys
    | PLet (x, e1, e2) ->
      let ty = go e1 in
...
let lambda_lift_sub
  ...
  let rec go (phi : tv_expression) : tv_expression = match phi with
    | PLambda (args, body) -> begin
      ...
      let bodyty = get_type body in
      let funty = to_funty (List.map (fun i -> i.Id.ty) args) bodyty in
...
let assign_id use_type_var_in_return_type (raw : R.raw_program) : tv_program =
  let open R in
  let pred_args =
    List.map (
      fun {var; args; body} ->
        ...
        let ty = to_funty (List.map snd args) (if use_type_var_in_return_type then (TVar (Id.gen ())) else TUnit) in
