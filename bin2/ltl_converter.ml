...
let main filepath show_style disable_optimization solve always_use_canonical_type_env encode_nondet_with_forall =
...
let command =
  Command.basic
    ...
    Command.Let_syntax.(
      ...
      and show_style =
        flag "--show-style" (optional_with_default Abbrev_id read_show_style) ~doc:"output id without escaping (for debug)"
      ...
      in
      (fun () -> main filepath show_style disable_optimization solve always_use_canonical_type_env encode_nondet_with_forall)
    )
